...
the set of functions used in number state reconstruction of light pulses, especially the
...
def constructRTmatrix(size, order, tD, bw, normedSignalData, window_width, backRefKeepOrder = 0, verbose = True, newStrArr=np.array([])):
    ...
    # Check the matrix diagonals and set them to match the constraint
    # that columns must sum to unity
    matSums = np.sum(mat, axis=0) - np.diag(mat)
...
def constructBCMatrix(pB, nMax, BCorder = 0, th = 1e-6):
    ...
    for i in range(nMax+1+BCorder):
        for j in range(nMax+1):
            if i==j:
                mat[i][j] = 1-np.sum([pB**(k)*np.exp(-pB)/sp.factorial(k) for k in np.arange(1,nMax+1+BCorder-j)])
                continue
            if i<j:
                mat[i][j] = 0
                continue
...
def constructAPMatrix(pA, nMax, APorder = 2):
    ...
    p_AP = np.append(1 - np.sum(np.array([pA**i for i in range(1, APorder+1)])), [pA**i for i in range(1, APorder+1)])
    
    for i in np.arange(1, nMax+1):
        for j in np.arange(i+1, min(i+APorder+1,nMax+1)):
            s = getSums(j-i, i, j)
            mat[j][i] = np.sum(np.product(p_AP[s], axis=1))
    for i in np.arange(1, nMax+1):
        mat[i][i] = 1-np.sum(np.transpose(mat)[i])
